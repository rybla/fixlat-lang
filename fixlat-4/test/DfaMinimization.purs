...
transition = do
  ...
  let lattice = PowerLatticeType ((state.data `TupleDataType` state.data) `TupleDataType` label.data)
  { name
  , lattice
  , data: toDataType lattice
...
makeModule (Dfa dfa) = 
  emptyModule # Newtype.over Module _
    ...
    , 
      fixpoints = 
        let axiomNames = Array.concat
              [ [Name "starting state"]
              , Array.mapWithIndex (\i _ -> makeTransitionAxiomName i) dfa.transitions
              , Array.mapWithIndex (\i _ -> makeAcceptingAxiomName i) (Set.toUnfoldable dfa.acceptingStates)
              ] 
        in
        Map.fromFoldable 
          [ _fix_reachability /\ FixpointSpec
              { axiomNames
              , ruleNames: [_starting_reachable, _transition_reachable] }
          , _fix_live /\ FixpointSpec
              { axiomNames
              , ruleNames: [_accepting_live, _transition_live] }
          ]
    }
  where
  makeTransitionAxiomName i = Name ("transition #" <> show i)
  makeAcceptingAxiomName i = Name ("accepting #" <> show i)
