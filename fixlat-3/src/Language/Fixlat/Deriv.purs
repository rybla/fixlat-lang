  ...
  comparePartial d1 d2 =
    case runUnify (unifyLeDeriv d1 d2) of
      Just sigma -> 
        ...
        if isRenaming sigma then Just EQ
...
type UnifyM xt = ReaderT (Map.Map xt Quant) (StateT (Map.Map xt (LTerm xt)) Maybe)

runUnify :: forall xt a. UnifyM xt a -> Maybe (Map.Map xt (LTerm xt))
runUnify = flip runReaderT Map.empty >>> flip runStateT Map.empty >>> map snd
...
unifyLeDeriv deriv1 deriv2 = do
  Deriv d1 <- substDeriv deriv1 <$> get
  Deriv d2 <- substDeriv deriv2 <$> get
...
unifyLeLTerm term1 term2 = do
  term1' <- substLTerm term1 <$> get
  term2' <- substLTerm term2 <$> get
  case term1' /\ term2' of
