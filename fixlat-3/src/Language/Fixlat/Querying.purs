...
{-
        ...
        k =<< (\f -> foldM f Nothing derivs) case _ of
          -- already proved goal, so just keep that finished derivation
          Just d -> \_ -> pure (Just d)
          -- haven't proved goal yet, so try this next derivation
          Nothing -> \deriv0 -> do
            ...
            ( unifyDerivationConclusionWithGoal deriv goal \(deriv' /\ goal') -> do
                ...
                -- failed to unify goal with derivation's conclusion
                -- !TODO move deriv to back
                Left err -> do
                  Debug.traceM $ "[queryProp] unify FAILURE:" <+> err
                  pure Nothing
                Right mb_deriv' -> pure mb_deriv'
-}
